# NAME
#        conso-hierarchy_week
#
# DESCRIPTION
#
#       'conso-hierarchy_week' is a script that returns one json file per week
#       (see nojour) in each region for each food group
#
# HISTORY
#
# 11 january 2018 - Initial design and coding. (Valentina Zelaya, @vz-chameleon, TGarwood @TGarwood)

import pandas as pd
import numpy as np
import os.path
import json
import io

# Interesting fields : codgr	libgr	sougr	libsougr	codal	libal
data_url_nomenclature='../raw/INCA2/Nomenclature_3.csv'
df_nomenclature=pd.read_csv(data_url_nomenclature, sep = ';')

for reg in range(1,22):
    for foodgrp in range(1,45):
        data_url_consoreg='../csv/conso_per_region/region'+str(reg)+'/full_conso_of_codgr_'+str(foodgrp)+'_region_'+str(reg)+'.csv'
        df_consoreg=pd.read_csv(data_url_consoreg, sep = ';')

        # Interesting fields : codgr;sougr;codal
        hierarchy_foogrp_region = df_nomenclature.loc[(df_nomenclature['codgr'] == foodgrp)]

        # JSON for hierarchy :)
        data = {}
        data["name"] = 'region_'+str(reg)+'_weekconso_foodgrp_'+str(foodgrp)
        weekday_list=[]
        for day in range(1,8):
            day_conso=df_consoreg.loc[(df_consoreg['nojour'] == day)]
            day_dict={}
            day_dict["name"]=str(day)+"e jour enregistrement"

            foodsougrp_list=[]
            for foodsougrp in np.unique(day_conso["sougr"].values):
                foodsougrp_dict={}
                # print('sousgrp'+str(foodsougrp))
                sougrp_region=hierarchy_foogrp_region.loc[(hierarchy_foogrp_region['sougr'] == foodsougrp)]
                sougrp_name=sougrp_region['libsougr'].iloc[0]
                foodsougrp_dict["name"]=sougrp_name

                sougrp_conso=day_conso.loc[(day_conso['sougr'] == foodsougrp)]
                conso_list=[]
                for codal in np.unique(sougrp_conso['codal'].values):
                    conso_codal={}
                    codal_region=sougrp_region.loc[(sougrp_region['codal'] == codal)]
                    codal_name=codal_region['libal'].iloc[0]
                    conso_codal["name"]=codal_name

                    c=sougrp_conso.loc[(sougrp_conso['codal'] == codal)]
                    conso_codal["size"]=c.qte_brute.mean()
                    conso_list.append(conso_codal)

                    foodsougrp_dict["children"]=conso_list
                foodsougrp_list.append(foodsougrp_dict)
                day_dict["children"]=foodsougrp_list

            weekday_list.append(day_dict)
        data["children"] = weekday_list

        file_path = os.path.join('..', 'json', 'foodgrp_sougrp_conso_week','region'+str(reg))
        file_name = 'consos_grp_'+str(foodgrp)+'_region_'+str(reg)+'.json'
        if not os.path.exists(file_path):
            os.makedirs(file_path)

        with open(os.path.join(file_path,file_name), 'w') as json_file:
            json.dump(data, json_file, ensure_ascii=False)
            # print(data)
