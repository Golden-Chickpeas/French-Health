# NAME
#        duree_(repas)_mean_by_tyrep_by_nojour_parser.py.py
#
# DESCRIPTION
#
#       'duree_(repas)_mean_by_tyrep_by_nojour_parser.py' is a script that returns one csv per tyrep
#       (see tyrep) per nojour (see nojour) containing the mean meal length of a type of meal for a specific day
#       of the week in all regions
#       We first retrieve all individuals in a given region and then, for each day (1 to 7), for each meal type (1 to 6)
...
for nojour in range(1,8):
    ...
    for tyrep in [1,3,5]:
        for reg_code in range(1,22):
            ...
            c=df_repas.loc[(df_repas['nojour'] == nojour) & (df_repas['tyrep'] == tyrep) & (df_repas['nomen'].isin(indiv_list))]
        ...
        file_name = 'duree_repas_mean_of_tyrep_'+type_repas[tyrep-1]+'_of_nojour_'+str(nojour)+'.csv'
    ...
    for revenu_foyer in range(1,16):
        ...
        for nojour in range(1,8):
            ...
            for tyrep in [1, 3, 5]:
                ...
                file_path = os.path.join('..','csv','duree_repas_mean')
                file_name = 'duree_repas_mean_of_tyrep_'+type_repas[tyrep-1]+'_of_nojour_'+str(nojour)+'_of_sexe_'+str(sexe_ps)+'_of_revenu_foyer_'+str(revenu_foyer)+'.csv'
                if not os.path.exists(file_path):
                    os.makedirs(file_path)
                df_regions.to_csv(os.path.join(file_path,file_name), sep=';')
    ...
    for nojour in range(1,8):
        ...
        for tyrep in [1, 3, 5]:
            for reg_code in range(1,22):
                ...
                df_indiv_reg = pd.read_csv(data_url_indiv_region, sep=';')
                df_indiv_reg_filtered = df_indiv_reg.loc[
                    (df_indiv_reg['nomen'].isin(indiv_list))]
            ...
            file_path = os.path.join('..','csv','duree_repas_mean')
            file_name = 'duree_repas_mean_of_tyrep_'+type_repas[tyrep-1]+'_of_nojour_'+str(nojour)+'_of_revenu_foyer_'+str(revenu_foyer)+'.csv'
            if not os.path.exists(file_path):
                os.makedirs(file_path)
            df_regions.to_csv(os.path.join(file_path,file_name), sep=';')
...
for sexe_ps in range(1,3):
    ...
    for nojour in range(1,8):
        ...
        for tyrep in [1, 3, 5]:
            ...
            file_path = os.path.join('..','csv','duree_repas_mean')
            file_name = 'duree_repas_mean_of_tyrep_'+type_repas[tyrep-1]+'_of_nojour_'+str(nojour)+'_of_sexe_'+str(sexe_ps)+'.csv'
            if not os.path.exists(file_path):
                os.makedirs(file_path)
            df_regions.to_csv(os.path.join(file_path,file_name), sep=';')
